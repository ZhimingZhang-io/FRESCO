...
from utils_loss import adaptive_feature_clip_loss
        ...
        for epoch_counter in tqdm(range(start_epoch, self.max_epochs+1)):
            ...
            for data in tqdm(train_loader):
                ...
                with autocast():
                    ...
                    proj_ecg_emb, proj_text_emb = output_dict['proj_ecg_emb'],\
                    ...
                    cma_loss, acc1, acc5 = adaptive_feature_clip_loss(agg_proj_img_emb, agg_proj_text_emb, temperature=0.07, soft_temperature=0.1, alpha=1, device=self.device)
                    # uma_loss, _, _ = clip_loss(agg_proj_ecg_emb1, agg_proj_ecg_emb2, device=self.device)
                    loss = cma_loss

                    if self.device == 0:
                        print(f'loss is {loss.item()}, acc1 is {acc1.item()}, acc5 is {acc5.item()}, cma_loss is {cma_loss.item()}')

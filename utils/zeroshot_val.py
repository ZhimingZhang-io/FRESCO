...
def compute_AUCs(gt, pred, n_class):
    ...
    for i in range(n_class):
        ...
        if len(unique_labels) == 1:
            ...
            pred_mean = np.mean(pred_class)
            
            if label_value == 1:
                # 全是正样本，好的模型应该给出高预测值
                pseudo_auc = max(pred_mean, 0.5)
                print(f"类别 {i}: 全正样本 (预测均值={pred_mean:.3f})，伪AUC={pseudo_auc:.3f}")
            else:
                # 全是负样本，好的模型应该给出低预测值  
                pseudo_auc = max(1.0 - pred_mean, 0.5)
                print(f"类别 {i}: 全负样本 (预测均值={pred_mean:.3f})，伪AUC={pseudo_auc:.3f}")
            
            AUROCs.append(pseudo_auc)
            
        else:
            # 正常计算AUC
            try:
                auc = roc_auc_score(gt_class, pred_class, average='macro', multi_class='ovo')
                AUROCs.append(auc)
                valid_aucs.append(auc)
            except ValueError as e:
                print(f"类别 {i} AUC计算失败: {e}，使用默认值0.5")
                AUROCs.append(0.5)
    
    # 报告统计信息
    if single_class_count > 0:
        print(f"⚠️  发现 {single_class_count}/{n_class} 个单类别标签，使用伪AUC")
    
    if valid_aucs:
        print(f"✅ {len(valid_aucs)}/{n_class} 个类别可正常计算AUC")
